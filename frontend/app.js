const demoEvents=[
{id:1,place:'Дзержинск',region:'Нижегородская область',source:'@nn52signal',type:'fix',status:'фиксация',lat:56.238,lon:43.46,published_at:'2026-08-22T10:00:00+03:00',confirmations:2,text:'Публичное сообщение о фиксации в населённом пункте.',direction_text:null},
{id:2,place:'Выкса',region:'Нижегородская область',source:'@radar_nizhniinovgorod',type:'cancel',status:'отбой / опровержение',lat:55.32,lon:42.17,published_at:'2026-08-21T22:20:00+03:00',confirmations:1,text:'Ранее опубликованное сообщение позднее было снято источником.',direction_text:null},
{id:3,place:'Семилуки',region:'Воронежская область',source:'@vrv_radar',type:'fix',status:'фиксация',lat:51.69,lon:39.03,published_at:'2026-08-21T19:40:00+03:00',confirmations:1,text:'Публичное сообщение о фиксации.',direction_text:null}
];
...
function directionLine(e){if(!e.direction_text)return '';return `<br><span class="direction">Направление по тексту источника: ${e.direction_text}</span>`;}
function showDetail(e){selectedId=e.id;detail.innerHTML=`<h2>Выбранная точка</h2><p><b>${e.place}</b></p><div class="meta">${e.region}<br>${e.status||e.type}<br>${ageText(ageMinutes(e))}<br>Источник: ${e.source}<br>Подтверждений: ${e.confirmations||1}${e.approximate?'<br>Координата населённого пункта/района приблизительная':''}${directionLine(e)}${e.text?`<br><br>${e.text}`:''}</div>`;document.querySelectorAll('.event').forEach(n=>n.classList.toggle('active',String(n.dataset.id)===String(e.id)));}
...
function render(){const items=filteredEvents();layer.clearLayers();markers.clear();feed.innerHTML='';items.sort((a,b)=>eventDate(b)-eventDate(a)).forEach(e=>{if(Number.isFinite(e.lat)&&Number.isFinite(e.lon)){const marker=L.marker([e.lat,e.lon],{icon:makeIcon(e)}).addTo(layer);marker.bindPopup(`<b>${e.place}</b><br>${e.region}<br>${e.status||e.type}<br>${e.source}<br>${ageText(ageMinutes(e))}${e.direction_text?`<br>Направление из публикации: ${e.direction_text}`:''}`);marker.on('click',()=>showDetail(e));markers.set(e.id,marker);}const div=document.createElement('button');div.type='button';div.className='event';div.dataset.id=e.id;div.innerHTML=`<b>${e.place}${(e.confirmations||1)>1?`<span class="badge">${e.confirmations} источника</span>`:''}</b><div class="meta">${e.region}<br>${e.status||e.type} · ${ageText(ageMinutes(e))}<br>${e.source}${e.direction_text?`<br>↗ ${e.direction_text}`:''}</div>`;div.addEventListener('click',()=>{if(markers.has(e.id)){map.setView([e.lat,e.lon],9);markers.get(e.id).openPopup();}showDetail(e);});feed.appendChild(div);});
 document.getElementById('events').textContent=items.length;document.getElementById('regions').textContent=new Set(items.map(e=>e.region)).size;document.getElementById('sources').textContent=new Set(items.map(e=>e.source)).size;document.getElementById('confirmed').textContent=items.filter(e=>(e.confirmations||1)>1).length;document.getElementById('visibleCount').textContent=`${items.length} точек`;document.getElementById('feedCount').textContent=`${items.length}`;
